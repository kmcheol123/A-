...
public class FindPathAStar : MonoBehaviour
{
    ...
    void BeginSearch()      // 맵 내무에서 시작 포인트와 종료 포인트를 만들어낸다.
    {
        ...
        }
        {
            ...
            mapLocation1.x = locations[0].x;
            mapLocation1.z = locations[0].z;

            Vector3 startLocation = new Vector3(locations[0].x * maze.scale, 0, locations[0].z * maze.scale);
            startNode = new PathMarker(mapLocation1, 0, 0, 0, Instantiate(start, startLocation, Quaternion.identity), null);
            MapLocation mapLocation2 = new MapLocation();
            mapLocation2.x = locations[1].x;
            mapLocation2.z = locations[1].z;
            // 리스트를 두번째를 시작지로 
            Vector3 goalLocation = new Vector3(locations[1].x * maze.scale, 0, locations[1].z * maze.scale);
            goalNode = new PathMarker(mapLocation2, 0, 0, 0, Instantiate(end, goalLocation, Quaternion.identity), null);
        ...
        void Search(PathMarker thisNode)
        {
            ...
            foreach (MapLocation dir in maze.directions)
            {
                ...
                float G = Vector2.Distance(new Vector2(thisNode.location.x,thisNode.location.z), new Vector2(thisNode.location.x, thisNode.location.z)) + thisNode.G;
                float H = Vector2.Distance(new Vector2(neighbor.x, neighbor.z), new Vector2(goalNode.location.x, goalNode.location.z));
                float F = G + H;
